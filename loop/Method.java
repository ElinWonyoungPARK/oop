...
public class Ex07Method {
     ...
     * 그 코드를 처음부터 끝까지 다시 써주는 것은 비효율적이고 귀찮은 노동이된다.
     * 
     ...
     * 선언: 해당 메소드의 이름, 그리고 실행할때 필요로 하는 외부으 값, 그리고 이 메소드가 종료되면서
     ...
     *           그 외의 경우에는 기능 구현 안에 반드시 return이라는 명형어를 통해서
     ...
     *          만약 해당 메소드를 실행하는데 필요로 하는 외부의 여러개면 여러개의 파라미터가
     *          들어간다.
     ...
     * 메소드 오버로딩인란, 이름은 같지만 파라미터가 달라져서 같은 목적을 가진
     ...
     * 그렇다면 add(1, 3.1) 혹은 ass(3.0, 1.4) 같이
     ...
     *  파라미터의 데이터타입 순서이기 때문에
     *  만약 printInfo("서울대") 라고 했을 때 1, 2, 3번 중 어떤게 호출되어야 하는가?
     *  정답은 컴퓨터가 정할 수 없다가 된다. 왜냐하면 1, 2, 3번 모두다 파라미터로 "스트링" 이기만 하면
     *  된다는 것이기 때문에 컴퓨터는 "서울대"가 사람이름이건, 학교이름이건, 주민등록번호이건 이런건 판단하지 
     ...
     *           이제는 외부가 정말저말 필요한 내용만 public 으로 해두고 나머지는 모두 다
     *           private으로 하는게 정석이다.
     *  
     *  
     *  
