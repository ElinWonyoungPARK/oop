 ...
 *      필요한 코드는 양을 줄이자가 바로 3세대 언어, 객체지향언어가 목표하는 바이다.
 *      
 *  2.  객체
 *      객체란, 클래스 변수를 뜻한다.
 *      즉 객체지향프로그래밍이란, 클래스 변수를 사용해서 그 변수가 생성하는 내부 메시지를 활용해서
 ...
 *      
 *  4.  필드
 *      필드란 해당 객체가 내부적으로 가지고 있는 데이터를 할당한 공간을 뜻한다.
 *      변수처럼
 *      데이터타입 필드이름
 *      으로 선언한다.
 ...
 *      파라미터 있는 생성자만 만들어서 외부의 값을 강제할 수도 있다!
 *      
 *  7.  필드와 파라미터의 호출 우선도
 ...
 *         그외에 메소드나 공간은 확보가 되어있는 상태이다! 즉 NullPointException은
 *         발생하지 않는다.
 ...
 *      B. equals()
 *         equals()의 경우 두 객체의 주소값이 아닌 필드에 저장된 값을 기준으로 해서
 *         두 객체가 같은지 아닌지를 비교한다.  
 *         하지만... java.lang.Object의 equals() 메소드는
 *                       return a == b;
 *         즉 주소값 비교의 결과값을 return 하도록 구현되어있다.
 *         만약 여러분들이 두 객체의 비교를 해야할 일이 있다면 (=데이터 담당 클래스일 경우)
 ...
 *      public void set 필드이름 (필드타입 필드이름){
 *          this.필드이름 = 필드이름;
 *      }
 *      
